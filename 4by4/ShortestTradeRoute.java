...
public class ShortestTradeRoute {
    ...
    //                                 .toArray(int[][]::new);
        ...
        // System.out.println("Runtime: " + runTime + " ns");
        Long runtimeDraw[] = new Long[10];
        Long BFSCost[] = new Long[10];
        for (int i = 0; i < 100; i++) {
            final int[][] test = Files.lines(FileSystems.getDefault().getPath("increasingRule30CA_4by4_"+ (i + 1) +".txt"))
                                    .map((l)->l.trim().split("\\s+"))
                                    .map((sa)->Stream.of(sa).mapToInt(Integer::parseInt).toArray())
                                    .toArray(int[][]::new);
            Long startTime = System.currentTimeMillis();
            Graph g = new Graph(test.length);
            g.drawGraph(test);

            runtimeDraw[i] = System.currentTimeMillis() - startTime;
            startTime = System.nanoTime();
            Dipath BFS = g.BreadthFirstSearch();
            BFSCost[i] = System.nanoTime() - startTime;
            System.out.println(BFSCost[i]);            
        } 
        BufferedWriter outputWriter = null;
        outputWriter = new BufferedWriter(new FileWriter(increasingRule30CA_4by4_BFS_runtime));
        outputWriter.write(Arrays.toString(runtimeDraw));
        outputWriter.flush();  
        outputWriter.close();  
    }
